...
function ErrorDisplay({errors, id}) {
  ...
  for(let field of splashInputs){
   ...
   if(field.id === 'review-message'){
      (errors.includes('message : Please enter a message.'))||errors.includes('message : Message character limit is 255 characters.')?
      field.classList.add('error-border'):field.classList.remove('error-border')
   }

   if(field.id === 'sign-up-firstname'){
      (errors.includes('first_name : Please enter a first name'))?
      field.classList.add('error-border'):field.classList.remove('error-border')
   }

   if(field.id === 'sign-up-lastname'){
      (errors.includes('last_name : Please enter a last name'))?
      field.classList.add('error-border'):field.classList.remove('error-border')
   }
